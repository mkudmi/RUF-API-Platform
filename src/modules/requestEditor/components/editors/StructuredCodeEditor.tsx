...
const editorHighlightStyle = HighlightStyle.define([
  { tag: tags.propertyName, class: 'cm-structured-key' },
  { tag: tags.number, class: 'cm-structured-number' },
  { tag: tags.string, class: 'cm-structured-string' },
  { tag: tags.bool, class: 'cm-structured-boolean' },
  { tag: tags.null, class: 'cm-structured-null' },
  { tag: tags.comment, class: 'cm-structured-comment' },
  { tag: tags.tagName, class: 'cm-structured-tag' },
  { tag: tags.attributeName, class: 'cm-structured-attribute' },
  { tag: [tags.attributeValue, tags.special(tags.string)], class: 'cm-structured-string' },
  { tag: [tags.angleBracket, tags.separator, tags.punctuation, tags.brace, tags.squareBracket, tags.paren], class: 'cm-structured-punctuation' },
])

const structuredEditorTheme = EditorView.theme({
  '&': {
    width: '100%',
  ...
  '.cm-content .cm-structured-key': {
    color: '#7fd8ff !important',
  },
  '.cm-content .cm-structured-number': {
    color: '#a9f5a9 !important',
  },
  '.cm-content .cm-structured-string': {
    color: '#ffb46a !important',
  },
  '.cm-content .cm-structured-boolean': {
    color: '#ff8ec2 !important',
  },
  '.cm-content .cm-structured-null': {
    color: '#b8c0cc !important',
  },
  '.cm-content .cm-structured-comment': {
    color: '#8aa0b8 !important',
  },
  '.cm-content .cm-structured-tag': {
    color: '#7fd8ff !important',
  },
  '.cm-content .cm-structured-attribute': {
    color: '#ffd479 !important',
  },
  '.cm-content .cm-structured-punctuation': {
...
export function StructuredCodeEditor(props: {
  value: string
  ...
  wrapVariablesInJsonStrings?: boolean
  ...
  const [editorHeight, setEditorHeight] = useState(() => {
    const measured = props.resizableOnMac ? measureDefaultBodyTextareaHeight() : 0
    const base = props.minHeight ?? (props.resizableOnMac ? 320 : 360)
    return Math.max(base, measured || 0)
  })
  ...
  useEffect(() => {
    if (props.resizableOnMac) return
    setEditorHeight(Math.max(220, props.minHeight ?? 360))
  }, [props.minHeight, props.resizableOnMac])

  const filtered = useMemo<VariableCompletion[]>(() => {
    const suggestions = props.variableSuggestions ?? []
    const vars = props.variables ?? {}
    const completions = getVariableCompletions(query, vars)
    if (query.trim() || completions.length !== suggestions.length) return completions
    return suggestions
  }, [props.variableSuggestions, props.variables, query])
  ...
  const updateVariableMenuFromView = useCallback((view: EditorView) => {
    if (!(props.variableSuggestions?.length)) {
      setMenuOpen(false)
      setQuery('')
      return
    }
    ...
    setMenuOpen(true)
  }, [props.variableSuggestions, updateAnchorFromPos])
  ...
  const applySuggestion = useCallback((name: string) => {
    ...
    const shouldWrapInJsonString = props.wrapVariablesInJsonStrings && props.format === 'json' && !isInsideJsonString(doc, token.start)
    const insert = shouldWrapInJsonString ? `"${placeholder}"` : placeholder
    const placeholderOffset = shouldWrapInJsonString ? 1 : 0
    const selection = getSuggestionSelectionRange(name, token.start, placeholderOffset)

    view.dispatch({
      changes: { from: token.start, to: tokenEnd, insert },
      selection: selection ?? { anchor: token.start + insert.length },
    })
    view.focus()
    setMenuOpen(false)
    setQuery('')
  }, [props.format, props.wrapVariablesInJsonStrings])
  ...
  const extensions = useMemo(() => {
    const out = [
      ...
      keymap.of([
        {
          key: 'Mod-Enter',
          run: () => props.onSubmitShortcut?.() ?? false,
          preventDefault: true,
        },
        {
          key: 'ArrowDown',
        ...
        },
        {
          ...
          run: () => {
            ...
            applySuggestion(picked.insertName ?? picked.name)
            return true
          },
          preventDefault: true,
        },
    ...
    return out
  }, [applySuggestion, props.format, props.onSubmitShortcut, updateVariableMenuFromView])
  ...
  useEffect(() => {
    ...
    const minHeight = Math.max(220, props.minHeight ?? (props.resizableOnMac ? 320 : 360))
    view.dom.classList.add('mono')
    view.dom.style.minHeight = `${minHeight}px`
    view.dom.style.height = `${Math.max(minHeight, editorHeight)}px`
    view.dom.style.marginTop = `${props.marginTop ?? 0}px`
    view.dom.style.marginBottom = `${props.marginBottom ?? 0}px`
    if (props.resizableOnMac) view.dom.style.resize = 'none'
  ...
  }, [extensions, props.marginBottom, props.marginTop, props.minHeight, props.resizableOnMac, updateVariableMenuFromView])
  ...
  useEffect(() => {
    ...
    const minHeight = Math.max(220, props.minHeight ?? (props.resizableOnMac ? 320 : 360))
    view.dom.style.minHeight = `${minHeight}px`
    view.dom.style.height = `${Math.max(220, editorHeight)}px`
    view.dom.style.marginTop = `${props.marginTop ?? 0}px`
    view.dom.style.marginBottom = `${props.marginBottom ?? 0}px`
  }, [editorHeight, props.marginBottom, props.marginTop, props.minHeight, props.resizableOnMac])
  ...
  useEffect(() => {
    ...
    try {
      ...
      logWarn('StructuredCodeEditor.scrollIntoView', 'Failed to scroll active variable suggestion into view', {
        error,
        activeIndex,
      })
    }
  ...
  const onResizeHandlePointerDown = useCallback((event: ReactPointerEvent<HTMLDivElement>) => {
    if (!IS_MAC || !props.resizableOnMac) return
    ...
    function onMove(nextEvent: PointerEvent) {
      const next = Math.max(140, startHeight + (nextEvent.clientY - startY))
  ...
  }, [editorHeight, props.resizableOnMac])
  ...
  return (
    <div>
        ...
        >
          {filtered.length ? (
            filtered.map((suggestion, idx) => (
              <button
                key={`${suggestion.kind}:${suggestion.insertName ?? suggestion.name}`}
                type="button"
                className={`selectMenuItem ${idx === activeIndex ? 'selectMenuItemActive' : ''}`}
                role="menuitem"
                data-var-idx={idx}
                onClick={() => applySuggestion(suggestion.insertName ?? suggestion.name)}
              >
                <div className="varMenuItem">
                  <div className="mono">{suggestion.name}</div>
                  {suggestion.description ? <div className="varMenuDesc">{suggestion.description}</div> : null}
                </div>
              </button>
            ))
          ) : (
      ...
      {IS_MAC && props.resizableOnMac ? <div className="bodyResizeHandle" onPointerDown={onResizeHandlePointerDown} /> : null}
    </div>
  )
}

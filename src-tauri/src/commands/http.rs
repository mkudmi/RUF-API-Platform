...
fn should_drop_header(name_lower: &str) -> bool {
  matches!(
    name_lower,
    "origin" | "referer" | "host" | "content-length" | "sec-fetch-mode" | "sec-fetch-site"
...
pub async fn http_request(args: HttpRequestArgs) -> Result<HttpResponseData, String> {
  async fn inner(args: HttpRequestArgs) -> Result<HttpResponseData, HttpError> {
    ...
    if url.to_ascii_lowercase().starts_with("http:/") && !url.to_ascii_lowercase().starts_with("http://") {
      url = url.replacen("http:/", "http://", 1);
    } else if url.to_ascii_lowercase().starts_with("https:/") && !url.to_ascii_lowercase().starts_with("https://") {
    ...
    let method = args
      ...
      .trim()
      .to_uppercase();
    let method = reqwest::Method::from_bytes(method.as_bytes()).map_err(|_| HttpError::InvalidMethod)?;
